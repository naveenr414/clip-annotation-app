...
def write_tokens(token_file,database):
    ...
    for qanta_id in by_qanta_id:
        c.execute("UPDATE QUESTIONS SET TOKENS=? WHERE QANTA_ID=?",[json.dumps(by_qanta_id[qanta_id]),qanta_id])
        if qanta_id%1000 == 0:
            print(qanta_id)
    ...
    for mention in mentions:
        ...
        for sentence in mention['mentions']:
            for entity in sentence:
                ...
                c.execute("INSERT INTO MENTIONS (ENTITY,QUESTION_ID,START,END,EDITED,SCORE) VALUES (?,?,?,?,0,?)",[name,question_id,start,end,score])
