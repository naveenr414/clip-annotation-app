...
async def write_entity(entity: Entity):
    ...
    for i in range(len(entity.word_numbers)):
        our_entities.append({'start':entity.word_numbers[i][0],'end':entity.word_numbers[i][1],'entity':entity.entities[i]})
    ...
    for i in range(len(old_entities)):
        if old_entities[i] not in our_entities and old_start_ends[i] in our_start_ends :
            ...
            update_entities.append((old_entity_ids[i],our_entities[our_start_ends.index(old_start_ends[i])]['entity']))
