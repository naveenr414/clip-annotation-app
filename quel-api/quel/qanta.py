...
def get_question(qanta_id: int):
    ...
    while i<len(new_tokens) and entity_pointer<len(question_dict["entities"]):
        ...
        while entity_pointer<len(question_dict["entities"]) and question_dict["entities"][entity_pointer]["start"] < new_tokens[i]['start']:
            entity_pointer+=1
        ...
        if question_dict["entities"][entity_pointer]["start"] == new_tokens[i]['start']:
            start = i
            while question_dict["entities"][entity_pointer]["end"] > new_tokens[i]['end']:
                i+=1
            end = i

            if(question_dict["entities"][entity_pointer]["score"] == -1 or question_dict["entities"][entity_pointer]["score"] >CUTOFF):    
